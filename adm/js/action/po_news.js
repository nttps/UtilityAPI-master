...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/poNewsController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                col_checkbox += '  value="' + item.i_news + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                if (item.s_img_p1 != "") {
                    col_picture = '<a title="' + item.s_img_p1 + '" class="example-image-link" href="upload/postnews/' + item.s_img_p1 + '" data-lightbox="example-' + item.i_news + '">';
                    col_picture += '<img class="example-image" src="upload/postnews/' + item.s_img_p1 + '" width="50px" height="50px"  />';
                    col_picture += '</a>';
                }
                ...
                col_edit += '<a href="po_news_manage.php?func=edit&id=' + item.i_news + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_news + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/poNewsController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/poNewsController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
