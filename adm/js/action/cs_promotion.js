...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/promotionCSController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                var col_checkbox = "";
                var col_detail_th = item.s_detail_th;
//                var col_detail_en = item.s_detail_en;
                var col_status = "";
                var col_edit = "";
                var col_delete = "";

                if (item.i_promotion != "1") {
                    ...
                    col_checkbox += '  value="' + item.i_promotion + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                if (item.i_promotion == "1") {
                    col_edit += '<a href="#" class="btn btn-circle btn-icon-only blue"  style="width:33px;height:33px" disabled="disabled">';
                } else {
                    col_edit += '<a href="' + (disable != "" ? '#' : "cs_promotion_manage.php?func=edit&id=" + item.i_promotion) + '" style="width:33px;height:33px" class="btn btn-circle btn-icon-only blue" ' + disable + '>';
                }
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';

                if (item.i_promotion == "1") {
                    col_delete += '<a href="#" class="btn btn-circle btn-icon-only red" disabled="disabled" style="width:33px;height:33px">';
                } else {
                    col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_promotion + '\',\'delete\');') + '" style="width:33px;height:33px" class="btn btn-circle btn-icon-only red" ' + disable + '>';
                }
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/promotionCSController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
