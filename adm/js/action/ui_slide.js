...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/uiSlideController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                col_checkbox += '  value="' + item.i_slide + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                if (item.i_type > 1) {
                    if (item.s_img_p1 != "") {
                        col_picture = '<a title="' + item.s_img_p1 + '" class="example-image-link" href="upload/slide/' + item.s_img_p1 + '" data-lightbox="example-' + item.i_slide + '">';
                        col_picture += '<img class="example-image" src="upload/slide/' + item.s_img_p1 + '" width="35px" height="35px"  />';
                        col_picture += '</a>';
                    }
                } else {
                    if (item.s_img_video != "") {
                        col_picture = '<a title="' + item.s_img_video + '" class="example-image-link" href="upload/slide/' + item.s_img_video + '" data-lightbox="example-' + item.i_slide + '">';
                        col_picture += '<img class="example-image" src="upload/slide/' + item.s_img_video + '" width="35px" height="35px"  />';
                        col_picture += '</a>';
                    }
                }
                ...
                col_edit += '<a href="ui_slide_manage.php?func=edit&id=' + item.i_slide + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="javascript:Confirm(\'' + item.i_slide + '\',\'delete\');" class="btn btn-circle btn-icon-only red">';
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/uiSlideController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/uiSlideController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
