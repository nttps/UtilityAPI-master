...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/registerController.php?func=dataTable',
        beforeSend: function ()
        {
...
//                    col_checkbox += ' </label>';
                ...
                col_edit += '<a href="cs_register_manage.php?func=edit&id=' + item.i_user + '" style="width:33px;height:33px" class="btn btn-circle btn-icon-only blue">';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/registerController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
