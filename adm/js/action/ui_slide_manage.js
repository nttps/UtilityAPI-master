...
function getDDLPointion() {
    $.ajax({
        type: 'GET',
        url: 'controller/commonUIController.php?func=DDLPointion',
        beforeSend: function ()
        {
            //$('#se-pre-con').fadeIn(100);
        },
...
function getDDLType() {
    $.ajax({
        type: 'GET',
        url: 'controller/commonUIController.php?func=DDLType',
        beforeSend: function ()
        {
            //$('#se-pre-con').fadeIn(100);
        },
...
function getDDLServer() {
    $.ajax({
        type: 'GET',
        url: 'controller/commonUIController.php?func=DDLServerSL',
        beforeSend: function ()
        {},
...
function changeConditon() {
    ...
    if (type == "1") {
        ...
        $('#div-img3').attr('style', 'display:block;');

        $('#div-img1').attr('style', 'display:none;');
        $('#div-img2').attr('style', 'display:none;');
        $("#div-img1 .fileinput").fileinput('clear');
        ...
        $('#div-img1').attr('style', 'display:block;');

        $('#div-img2').attr('style', 'display:none;');
        $('#div-sv').attr('style', 'display:none;');
        $('#div-sv-src').attr('style', 'display:none;');
        $("#div-img2 .fileinput").fileinput('clear');
        $('#div-img3').attr('style', 'display:none;');
        $("#div-img3 .fileinput").fileinput('clear');
...
function edit() {
    $.ajax({
        type: 'GET',
        url: 'controller/uiSlideController.php?func=getInfo&id=' + keyEdit,
        beforeSend: function ()
        {
            //$('#se-pre-con').fadeIn(100);
        },
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                if (item.s_img_p1 != "") {
                    $('#img1').attr('src', 'upload/slide/' + item.s_img_p1);
                    $("#tmp_img_p1").val(item.s_img_p1);
                }
                if (item.s_img_p2 != "") {
                    $('#img2').attr('src', 'upload/slide/' + item.s_img_p2);
                    $("#tmp_img_p2").val(item.s_img_p2);
                }
                if (item.s_img_video != "") {
                    $('#img3').attr('src', 'upload/slide/' + item.s_img_video);
                    $("#tmp_img_p3").val(item.s_img_video);
                }
