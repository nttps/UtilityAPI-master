...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/promotionRGCSController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                if (item.i_promotion == "1") {
                    col_edit += '<a href="#" class="btn btn-circle btn-icon-only blue"  disabled="disabled">';
                } else {
                    col_edit += '<a href="' + (disable != "" ? '#' : "cs_promotion_rg_manage.php?func=edit&id=" + item.i_promotion) + '" class="btn btn-circle btn-icon-only blue" ' + disable + '>';
                }
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';

                if (item.i_promotion == "1") {
                    col_delete += '<a href="#" class="btn btn-circle btn-icon-only red" disabled="disabled">';
                } else {
                    col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_promotion + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
                }
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/promotionRGCSController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/promotionRGCSController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
