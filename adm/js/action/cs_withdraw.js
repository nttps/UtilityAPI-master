...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/withdrawController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                col_checkbox += '  value="' + item.i_wd + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                col_edit += '<a href="cs_withdraw_manage.php?func=edit&id=' + item.i_wd + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_wd + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/withdrawController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
