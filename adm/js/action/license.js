...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/licenseController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                var col_checkbox = "";
                var col_domain =  item.s_domain;
                var col_key = item.s_license_key;
                var col_function = (language == "th" ? item.s_th : item.s_en);
                var col_status = "";
                var col_edit = "";
                var col_delete = "";
                ...
                col_checkbox += '  value="' + item.i_license + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                col_edit += '<a href="license_manage.php?func=edit&id=' + item.i_license + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_license+ '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/licenseController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
