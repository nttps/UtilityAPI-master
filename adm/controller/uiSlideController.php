...
$controller = new uiSlideController();
switch ($info[func]) {
    case "dataTable":
...
class uiSlideController {

    public function dataTable() {
        include '../service/uiSlideService.php';
        $service = new uiSlideService();
        ...
        if ($info[data] == NULL) {
            ...
            if ($service->deleteAll($db, $query)) {
                ...
                foreach ($arr_img as $key => $value) {
                    ...
                    if ($arr_img[$key]['s_img_video'] != NULL && $arr_img[$key]['s_img_video'] != "") {
                        $upload->add_FileName($arr_img[$key]['s_img_video']);
                    }
                }
                if (count($upload->get_Filename()) > 0) {
                    if ($upload->deleteFile()) {
    ...
    public function edit($info) {
        if ($this->isValid($info)) {
            ...
            if ($info[i_type] == "1") {
                //START CASE:1
                if ($_FILES["s_img_video"]["error"] == 4) {
                    if ($service->edit($db, $info, NULL, NULL, $info[tmp_img_p3])) {
                        $db->commit();
                        echo $_SESSION['cd_0000'];
                    } else {
                        $db->rollback();
                ...
                } else if ($_FILES["s_img_video"]["error"] == 0) {
                    $doc->add_FileName($_FILES["s_img_video"]);
                    $flg = $doc->AddFile();
                    if ($flg) {
                        $tmpDoc = $doc->get_FilenameResult();
                        if ($service->edit($db, $info, NULL, NULL, $tmpDoc[0])) {
                            $db->commit();
                            echo $_SESSION['cd_0000'];
                        } else {
                            $db->rollback();
    ...
    public function add($info) {
        if ($this->isValid($info)) {
            ...
            if ($info[i_type] == "1") {
                ...
                if ($flg) {
                    $tmpDoc = $doc->get_FilenameResult();
                    if ($service->add($db, $info, NULL, NULL, $tmpDoc[0])) {
                        $db->commit();
                        echo $_SESSION['cd_0000'];
                    } else {
                        $db->rollback();
    ...
    public function isValid($info) {
        ...
        if ($info[i_type] == "1") {
            ...
            if ($util->isEmpty($info[s_src_media])) {
                ...
                echo $return2099;
            } else if (($_FILES["s_img_video"]["error"] == 4 || $_FILES["s_img_video"] == NULL) && $info[func] == "add") {
