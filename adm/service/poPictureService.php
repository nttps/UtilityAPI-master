...
class poPictureService {
    ...
    function getInfo($seq) {
        require_once('../common/ConnectDB.php');
        $db = new ConnectDB();
        $strSql = " select * from tb_ps_img where i_img =" . $seq;
        $_data = $db->Search_Data_FormatJson($strSql);
        $db->close_conn();
        return $_data;
    }
    ...
    function getInfoFile($db) {
        $strSql = "select s_img_p1 img from tb_ps_img ";
        $strSql .= " union ";
    ...
    function delete($db, $seq) {
        $strSQL = "DELETE FROM tb_ps_img WHERE i_img = '" . $seq . "' ";
        $arr = array(
            array("query" => "$strSQL")
        );
    ...
    function deleteAll($db, $query) {
        ...
        $strSQL = "DELETE FROM tb_ps_img WHERE i_img in ($query) ";
        $arr = array(
            array("query" => "$strSQL")
        );
        $reslut = $db->insert_for_upadte($arr);
    ...
    function SelectByArray($db, $query) {
        $strSql = "SELECT * FROM tb_ps_img WHERE i_img in ($query) ";
        $_data = $db->Search_Data_FormatJson($strSql);
        return $_data;
    }
