...
class uiSlideService {
    ...
    function getInfo($seq) {
        require_once('../common/ConnectDB.php');
        $db = new ConnectDB();
        $strSql = " select * from tb_ui_slide where i_slide =" . $seq;
        $_data = $db->Search_Data_FormatJson($strSql);
        $db->close_conn();
        return $_data;
    }
    ...
    function getInfoFile($db) {
        $strSql = "select s_img_p1 img from tb_ui_slide ";
        $strSql .= " union ";
        $strSql .= " select s_img_p2 img from tb_ui_slide ";
        $strSql .= " union ";
        $strSql .= " select s_img_video img from tb_ui_slide ";
        $_data = $db->Search_Data_FormatJson($strSql);
        return $_data;
    }

    function delete($db, $seq) {
        $strSQL = "DELETE FROM tb_ui_slide WHERE i_slide = '" . $seq . "' ";
        $arr = array(
            array("query" => "$strSQL")
        );
    ...
    function deleteAll($db, $query) {
        ...
        $strSQL = "DELETE FROM tb_ui_slide WHERE i_slide in ($query) ";
        $arr = array(
            array("query" => "$strSQL")
        );
        $reslut = $db->insert_for_upadte($arr);
    ...
    function SelectByArray($db, $query) {
        $strSql = "SELECT * FROM tb_ui_slide WHERE i_slide in ($query) ";
        $_data = $db->Search_Data_FormatJson($strSql);
        return $_data;
    }
