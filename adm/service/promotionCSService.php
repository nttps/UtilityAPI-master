...
class promotionCSService {
    ...
    function getInfo($seq) {
        require_once('../common/ConnectDB.php');
        $db = new ConnectDB();
        $strSql = " select * from tb_cs_promotion where i_promotion =" . $seq;
        $_data = $db->Search_Data_FormatJson($strSql);
        $db->close_conn();
        return $_data;
    }
    ...
    function delete($db, $seq) {
        $strSQL = "DELETE FROM tb_cs_promotion WHERE i_promotion = '" . $seq . "' ";
        $arr = array(
            array("query" => "$strSQL")
        );
    ...
    function deleteAll($db, $query) {
        ...
        $strSQL = "DELETE FROM tb_cs_promotion WHERE i_promotion in ($query) ";
        $arr = array(
            array("query" => "$strSQL")
        );
        $reslut = $db->insert_for_upadte($arr);
